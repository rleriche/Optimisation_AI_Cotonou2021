...
\begin{block}{}
\begin{algorithmic}
\REQUIRE $f()$, $\alpha \in ]0,1]$, $x^1$, $\epsilon^{\text{step}}$, $\epsilon^{\text{grad}}$, $t^{\text{max}}$
\STATE $t\leftarrow 0$, $f^{\text{best so far}} \leftarrow $ \texttt{max\_double}
\REPEAT 
\STATE $t \leftarrow t+1$
\STATE calculate $f(x^t)$ and $\grad f(x^t)$
\IF{$f(x^t)< f^{\text{best so far}}$} 
\STATE update $x^{\text{best so far}}$ and $f^{\text{best so far}}$ with current iterate
\ENDIF
\STATE \textbf{direction: } $d^t = - \grad f(x^t) / \lVert \grad f(x^t) \rVert$
\STATE \textbf{step: } \textcolor{red}{$x^{t+1} ~=~ x^t + \alpha \lVert \grad f(x^t) \rVert d^t$}
\UNTIL{ {$t>t^{\text{max}}$} \OR {$\lVert x^t - x^{t-1}\rVert \le \epsilon^{\text{step}}$} \OR {$\lVert \grad f(x^t)\rVert \le \epsilon^{\text{grad}}$} }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Steepest descent with line search} 
oscillations thus need better directions
\wip
\end{frame}
