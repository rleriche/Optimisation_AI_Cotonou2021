...
$\mathcal S$ is an hypercube of $\Rset[d]$, $\mathcal S = [LB,UB] \in \Rset[d]$.
\vskip\baselineskip
It could be described by constraints, 
$g_{2i-1}(x) \coloneqq LB_i - x_i \le 0$,
$g_{2i}(x) \coloneqq x_i - UB_i \le 0$, $i=1,\ldots,d$
but these constraints are so simple that they directly handled by projection.\\
\mbox{
\begin{minipage}[c]{0.4\textwidth}
...
We give two techniques to agregate $f$ and the $g_i$'s into a new objective function 
(to minimize).
\vskip\baselineskip
...
$f$ and $g$ in black, $L(x)=f(x)+\lambda^\star g(x)$ with $\lambda^\star=4$ in red, 
